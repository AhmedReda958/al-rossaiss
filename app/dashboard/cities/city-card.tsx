...
export default function CityCard({ city, onCityDeleted }: CityCardProps) {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      let successMessage = 'City deleted successfully';
      if (projectsCount > 0 || landmarksCount > 0) {
        const deletedItems = [];
        if (projectsCount > 0) deletedItems.push(`${projectsCount} project${projectsCount > 1 ? 's' : ''}`);
        if (landmarksCount > 0) deletedItems.push(`${landmarksCount} landmark${landmarksCount > 1 ? 's' : ''}`);
        successMessage = `City and ${deletedItems.join(' and ')} deleted successfully`;
      }
  ...
  return (
    <>
      ...
      <ConfirmDeleteDialog
        ...
        title="Delete City"
        description={`Are you sure you want to delete "${city.name}"? This will also delete all associated projects and landmarks. This action cannot be undone.`}
